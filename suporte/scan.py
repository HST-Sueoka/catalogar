...
from suporte.format import process_docx, process_epub, process_pdf, process_txt, process_unknown
...
def escanear_diretorio(address, flag):
    ...
    for file_path in pathlib.Path(directory).rglob('*'):
        ...
        if nome_arquivo.endswith('.epub'):
            autor, titulo, idioma = process_epub(file_path, nome_arquivo)

        elif nome_arquivo.endswith('.pdf'):
            autor, titulo, idioma = process_pdf(file_path, nome_arquivo)

        elif nome_arquivo.endswith('.docx'):
            autor, titulo, idioma = process_docx(file_path, nome_arquivo)

        elif nome_arquivo.endswith('.txt'):
            autor, titulo, idioma = process_txt(file_path, nome_arquivo)

        else:
            autor, titulo, idioma = process_unknown(file_path, nome_arquivo)

        autor_normalizado = normalizar_palavra(autor)
        titulo_normalizado = normalizar_palavra(titulo)
    ...
    commit_scan(colecao_livros, colecao_autores, colecao_livros_normalizados, address, flag)
