...
def exportar_dados_root(address):
    ...
    try:
        ...
        query = "SELECT * from tb_autores order by autor"
        ...
        with open(os.path.join(diretorio_atual, "root_autores.json"), "w", encoding="utf-8") as file:
        ...
        query = "SELECT id, autor, titulo, idioma FROM tb_livros ORDER BY autor ASC, titulo ASC, idioma ASC, id ASC"
        cursor.execute(query)
        ...
        with open(os.path.join(diretorio_atual, "root_livros.json"), "w", encoding="utf-8") as file:
